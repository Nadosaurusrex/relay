...
class RelayStack(Stack):
    ...
    def _create_policy_bucket(self) -> s3.Bucket:
        ...
        bucket = s3.Bucket(
            self,
            "PolicyBucket",
            bucket_name=f"relay-policies-{self.account}-{self.env_name}",
            versioned=True,
    ...
    def _create_monitoring(self):
        ...
        dashboard.add_widgets(
            cloudwatch.GraphWidget(
                title="Request Count",
                left=[
                    self.ecs_service.target_group.metric_request_count(),
                ],
            ),
            cloudwatch.GraphWidget(
                title="Target Response Time",
                left=[
                    self.ecs_service.target_group.metric_target_response_time(),
                ],
            ),
        )
        ...
        dashboard.add_widgets(
            cloudwatch.GraphWidget(
                ...
                left=[
                    self.ecs_service.load_balancer.metric_http_code_target(
                        code=ecs_patterns.ApplicationLoadBalancerTargetGroupMetricOptions.HTTP_CODE_TARGET_5XX_COUNT
                    ),
                ],
            ),
            cloudwatch.GraphWidget(
        ...
        cloudwatch.Alarm(
            self,
            "HighErrorRateAlarm",
            metric=self.ecs_service.load_balancer.metric_http_code_target(
                code=ecs_patterns.ApplicationLoadBalancerTargetGroupMetricOptions.HTTP_CODE_TARGET_5XX_COUNT
            ),
            threshold=10,
        ...
        cloudwatch.Alarm(
            ...
            metric=self.ecs_service.target_group.metric_target_response_time(),
